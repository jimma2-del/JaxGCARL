...
#
# Copy of Brax's collisions.py in the spring backend - modified to allow custom friction scaling
...
def resolve(sys: System, state: State, friction_scale: float ) -> Motion:
  ...
  def impulse(c, link_idx, x_i, xd_i, i_inv, i_mass):
    ...
    is_floor = (link_idx[0] == -1) | (link_idx[1] == -1)
    friction_scale_eff = jp.where(is_floor, friction_scale, 1.0)
    friction_coeff = jp.where(is_floor, c.friction[0], 1.0)
    # drag magnitude cannot exceed max friction
    impulse_d = jp.minimum(friction_scale_eff * impulse_d, friction_coeff * impulse)
