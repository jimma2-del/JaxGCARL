...
def update_actor_and_alpha(config, networks, transitions, training_state, key, is_antag):
    ...
    def alpha_loss(alpha_params, log_prob):
        ...
        alpha_loss_value = alpha * jnp.mean(jax.lax.stop_gradient(-log_prob - config["target_entropy"]))
        return jnp.mean(alpha_loss_value)
    ...
    if (is_antag):
        actor_grad = jax.tree_util.tree_map(lambda g: -g, actor_grad)
...
def update_critic(config, networks, transitions, training_state, key, is_antag):
    def critic_loss(critic_params, transitions, key):
        sa_encoder_params, g_encoder_params = ( critic_params["sa_encoder"],  critic_params["g_encoder"],)
        state = transitions.observation[:, : config["state_size"]]
        
        action = transitions.antag_action if is_antag else transitions.protag_action
        sa_repr = networks["sa_encoder"].apply(sa_encoder_params, jnp.concatenate([state, action], axis=-1) )
