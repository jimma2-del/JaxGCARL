...
if GCARL:
    def update_actor_and_alpha(config, networks, transitions, training_state, key):
        def actor_loss(actor_params, critic_params, log_alpha, transitions, key):
            ...
            op_action = transitions.extras["other_actions"]
            ...
            sa_repr = networks["sa_encoder"].apply(
                sa_encoder_params, jnp.concatenate([state, action, op_action], axis=-1)
            )
    ...
    def update_op_actor_and_alpha(config, networks, transitions, training_state, key):
        def op_actor_loss(actor_params, critic_params, log_alpha, transitions, key):
            ...
            goal = future_state[:, config["goal_indices"]]
            observation = jnp.concatenate([state, goal], axis=1)
            action = transitions.extras["other_actions"]
            ...
            sa_repr = networks["sa_encoder"].apply(
                sa_encoder_params, jnp.concatenate([state, op_action, action], axis=-1)
            )
            g_repr = networks["g_encoder"].apply(g_encoder_params, goal)
        ...
        def op_alpha_loss(alpha_params, log_prob):
            alpha = jnp.exp(alpha_params["log_alpha"])
            alpha_loss = alpha * jnp.mean(
                jax.lax.stop_gradient(-log_prob - config["op_target_entropy"])
            )
            return jnp.mean(alpha_loss)
    
        (actor_loss, log_prob), actor_grad = jax.value_and_grad(op_actor_loss, has_aux=True)(
            training_state.actor_state.params,
            training_state.critic_state.params,
            training_state.alpha_state.params["log_alpha"],
            transitions,
        ...
        new_actor_state = training_state.actor_state.apply_gradients(grads=jax.tree_map(lambda g: -g, actor_grad))
    
        alpha_loss, alpha_grad = jax.value_and_grad(op_alpha_loss)(
            training_state.alpha_state.params, log_prob
        )
        new_alpha_state = training_state.alpha_state.apply_gradients(grads=alpha_grad)
...
if GCARL:
    ...
    def update_op_critic(config, networks, transitions, training_state, key):
        def op_critic_loss(critic_params, transitions, key):
            sa_encoder_params, g_encoder_params = (
                critic_params["sa_encoder"],
                critic_params["g_encoder"],
            )
